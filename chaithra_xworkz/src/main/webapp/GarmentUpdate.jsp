...
<body>
	...
	<h1>Welcome to Garment Update</h1>
	...
	<form action="update" method="post">
		...
		Location<input type="text" name="location" value="${dto.location}"
				required="required" />
		PhNo<input type="number" name="phNo" value="${dto.phNo }" readonly
				required="required" />
		OwnerName<input type="text" name="ownerName" value="${dto.ownerName }"
				readonly required="required" />
		NoOfEmployees<select name="noOfEmployees" required="required" />
		<option value="${dto.noOfEmployees }">${dto.noOfEmployees}</option>
